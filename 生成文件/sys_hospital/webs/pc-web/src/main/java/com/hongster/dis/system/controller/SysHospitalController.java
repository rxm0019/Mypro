 ...
 * SysHospital的控制器业务处理页面
 */

@Controller
@RequestMapping(value = "sysHospital")
public class SysHospitalController extends BaseController{
    public static final Logger logger = LoggerFactory.getLogger(SysHospitalController.class);
     ...
     * @return
     */
    @RequestMapping(value = "/sysHospitalList",method = RequestMethod.GET)
    public String sysHospitalList(ModelMap model){
        //此处可返回数据页面预处理...
        return "/system/sysHospitalList";
    }
     ...
     * @return
     */
    @RequestMapping(value = "/sysHospitalEdit",method = RequestMethod.GET)
    public String sysHospitalEdit(ModelMap model){
        //此处可返回数据页面预处理...
        return "/system/sysHospitalEdit";
    }
    ...
    @ResponseBody
    public Result getLists(HttpServletRequest request,SysHospital sysHospital){
        Result result = new Result();
        SysHospitalExample example = new SysHospitalExample();
        SysHospitalExample.Criteria criteria = example.createCriteria();
        List<SysHospital> list=iSysHospitalService.selectByExample(example);
    ...
    @ResponseBody
    public PageInfo list(HttpServletRequest request,  SysHospitalList sysHospitalList){
        Result result = new Result();
        PageInfo pageInfo=iSysHospitalService.list(sysHospitalList);
        return pageInfo;
    }
    ...
    @ResponseBody
    public Result getInfo(HttpServletRequest request,  SysHospitalEdit sysHospitalEdit){
        Result result = new Result();
        if(ToolUtil.isEmpty(sysHospitalEdit.getHospitalId())){
            throw new InvalidParamException(MessageHelper.ID_ISNULL);  //抛出异常
        }
        SysHospitalEdit sysHospital=iSysHospitalService.getInfo(sysHospitalEdit.getHospitalId());
        result.setBizData(sysHospital);
        return result;
    }
    ...
    @ResponseBody
    public Result saveOrEdit(HttpServletRequest request,  SysHospitalEdit sysHospitalEdit){
        Result result = new Result();
        if(sysHospitalEdit==null){
            throw new InvalidParamException(MessageHelper.MODEL_ISNULL);  //抛出异常
        }
        int code=iSysHospitalService.saveOrEdit(sysHospitalEdit);
    ...
    public Result delete(HttpServletRequest request,@RequestParam("ids[]") String[] ids){
        ...
        int code=iSysHospitalService.delete(ids);
        result.setBizData(code);
        return result;
    }
    ...
    @ResponseBody
    public Result exportExcel(HttpServletRequest request, String excelConfig,SysHospitalList sysHospitalList){
        Result result = new Result();
        ...
        //获取全部结果列表
        List<SysHospitalList> list=iSysHospitalService.listAll(sysHospitalList);
        //定义excel的结果变量
        List<Map<String, Object>> excelData = new ArrayList<>();
        if(list!=null&&list.size()>0){
        for (SysHospitalList dataList:list) {
