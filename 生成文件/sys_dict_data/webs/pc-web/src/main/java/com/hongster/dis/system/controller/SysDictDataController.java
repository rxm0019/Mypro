...
public class SysDictDataController extends BaseController{
     ...
     * @return
     */
    @RequestMapping(value = "/sysDictDataList",method = RequestMethod.GET)
    public String sysDictDataList(ModelMap model){
        //此处可返回数据页面预处理...
        return "/system/sysDictDataList";
    }
     ...
     * @return
     */
    @RequestMapping(value = "/sysDictDataEdit",method = RequestMethod.GET)
    public String sysDictDataEdit(ModelMap model){
        //此处可返回数据页面预处理...
        return "/system/sysDictDataEdit";
    }
    ...
    @ResponseBody
    public Result exportExcel(HttpServletRequest request, String excelConfig,SysDictDataList sysDictDataList){
        Result result = new Result();
        ...
        //获取全部结果列表
        List<SysDictDataList> list=iSysDictDataService.listAll(sysDictDataList);
        //定义excel的结果变量
        List<Map<String, Object>> excelData = new ArrayList<>();
        if(list!=null&&list.size()>0){
        for (SysDictDataList dataList:list) {
