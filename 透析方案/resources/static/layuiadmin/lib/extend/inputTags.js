  ...
  Class.prototype.init = function(){
    ...
    $.each(options.content,function(index,item){
      ...
      if (item.tagContent){
          ...
          tagColor = item.tagColors;
          tagStyle = 'background-color: #fff;border: 1px solid ' +tagColor+ ';color: '+tagColor+'';
      } else{
          tagContent = item;
      }
      spans +='<span style="' + tagStyle +'"><em>'+tagContent+'</em><button style="color: '+ tagColor +'" type="button" class="close">×</button></span>';
      // $('<div class="layui-flow-more"><a href="javascript:;">'+ ELEM_TEXT +'</a></div>');
    })
    //  layui.link(layui.setter.base +'lib/extend/' + 'inputTags.css');
...
}).link(layui.setter.base +'lib/extend/' + 'inputTags.css');
