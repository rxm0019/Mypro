/**
 * doctorTransfer.jsp的js文件，包括列表查询、排序、增加、修改、删除基础操作
 * 医生交班报表
 * @Author xcj
 * @version: 1.0
 * @Date 2020/10/17
 */
var doctorTransfer = avalon.define({
    $id: "doctorTransfer",
...
function initSearch(){
    _initSearch({
        elem: '#doctorTransfer_search' //指定搜索框表单的元素选择器（推荐id选择器）
        ,filter:'doctorTransfer_search'  //指定的lay-filter
        ,conds:[
        ...
        ,search:function(data){
            ...
            table.reload('doctorTransfer_table',{
                where:field
            });
        }
    });
...
function getHospitalAndUser() {
    var param = {
        userType:$.constant.userType.doctor
    }
    _ajax({
        type: "POST",
        ...
        dataType: "json",
            ...
            if(data.length>0){
                doctorTransfer.SysHospitalList.pushArray(data);
                var userList = [];
                //清空数据，重新绑定值
                var htmlHospital ='';
                $.each(doctorTransfer.SysHospitalList,function(i,item){
                    htmlHospital+='<option value="'+item.hospitalNo+'">'+item.hospitalName+'</option>';
                    if(doctorTransfer.baseFuncInfo.userInfoData.hospitalNo == item.hospitalNo){
                        userList = item.sysUserList;
                    }
                });
                $("select[name='hospitalNo']").html(htmlHospital);
                $("select[name='hospitalNo']").val(doctorTransfer.baseFuncInfo.userInfoData.hospitalNo);
...
function getList() {
    ...
    var param = {
        shiftDate_begin:util.toDateString(new Date(),"yyyy-MM-dd"),
        shiftDate_end:util.toDateString(new Date(),"yyyy-MM-dd"),
        hospitalNo:doctorTransfer.baseFuncInfo.userInfoData.hospitalNo
    };
    _layuiTable({
        elem: '#doctorTransfer_table', //必填，指定原始表格元素选择器（推荐id选择器）
        filter:'doctorTransfer_table', ////必填，指定的lay-filter的名字
        //执行渲染table配置
        render:{
            height:'full-140', //table的高度，页面最大高度减去差值
            url:  $.config.services.dialysis + "/patReport/getDoctorTransfer.do", // ajax的url必须加上getRootPath()方法
...
function exportExcel(){
    ...
    debugger
    var name = "医生交班报表_" + data.shiftDate_begin + "-" + data.shiftDate_end + ".xlsx";
    _downloadFile({
        url: $.config.services.dialysis + "/patReport/exportDoctorTransfer.do",
        data:data,
        fileName: name
    });
}
...
function getSearchParam() {
    var searchParam = layui.form.val("doctorTransfer_search");
