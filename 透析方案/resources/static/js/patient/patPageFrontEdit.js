/**
 * patPageFrontEdit.jsp的js文件，包括查询，编辑操作
 */
var patPageFrontEdit = avalon.define({
    $id: "patPageFrontEdit",
...
layui.use(['index'], function () {
    ...
    avalon.ready(function () {
        ...
        patPageFrontEdit.adjustType = adjustType;
        patPageFrontEdit.patientId = patientId;
        //获取实体信息
...
function verify_form($callback) {
    //监听提交,先定义个隐藏的按钮
    var form = layui.form; //调用layui的form模块
    form.on('submit(patPageFrontEdit_submit)', function (data) {
        //通过表单验证后
        var field = data.field; //获取提交的字段
        typeof $callback === 'function' && $callback(field); //返回一个回调事件
    });
    $("#patPageFrontEdit_submit").trigger('click');
}
...
function save($callback) {  //菜单保存操作
    ...
    verify_form(function (field) {
        ...
        if (!isEmpty(patPageFrontEdit.patientId)) {
            var key = 'patientId';
            var value = patPageFrontEdit.patientId
            param[key] = value;
            var adjustTypeKey = 'adjustType';
            var adjustValue = patPageFrontEdit.adjustType
