/**
 * bacClassDutyNurseReport.jsp的js文件，包括列表查询、排序、增加、修改、删除基础操作
 * 护士考勤统计报表页面
 * @Author xcj
 * @version: 1.0
 * @Date 2020/8/17
 */
var bacClassDutyNurseReport = avalon.define({
    $id: "bacClassDutyNurseReport",
...
function initSearch(){
    _initSearch({
        elem: '#bacClassDutyNurseReport_search' //指定搜索框表单的元素选择器（推荐id选择器）
        ,filter:'bacClassDutyNurseReport_search'  //指定的lay-filter
        ,conds:[
...
function getList(param) {
    ...
    _ajax({
        type: "POST",
        url: $.config.services.schedule + "/bacClassDuty/reportClassDutyNurse.do",
        dataType: "json",
        data:param,
        done:function(data){
            // 列表表头
            ...
            columnList.push({field: 'overTotal', title: '超缺工时(小时)', align:'center'});
            ...
            _layuiTable({
                elem: '#bacClassDutyNurseReport_table', //必填，指定原始表格元素选择器（推荐id选择器）
                filter:'bacClassDutyNurseReport_table', ////必填，指定的lay-filter的名字
                //执行渲染table配置
                render:{
...
function exportExcel(){
    ...
    var name = "护士考勤统计表_" + dutyMonth + ".xlsx";
    _downloadFile({
        url: $.config.services.schedule + "/bacClassDuty/exportClassDutyNurse.do",
