/**
 * bacClassDutyDoctorList.jsp的js文件，包括列表查询、排序、增加、修改、删除基础操作
 * 医生排班主页面
 * @Author xcj
 * @version: 1.0
 * @Date 2020/8/8
 */
var bacClassDutyDoctorList = avalon.define({
    $id: "bacClassDutyDoctorList",
...
function initSearch(){
    _initSearch({
        elem: '#bacClassDutyDoctorList_search' //指定搜索框表单的元素选择器（推荐id选择器）
        ,filter:'bacClassDutyDoctorList_search'  //指定的lay-filter
        ,conds:[
...
function getList(param) {
    ...
    param.dutyDate = "";
    _layuiTable({
        elem: '#bacClassDutyDoctorList_table', //必填，指定原始表格元素选择器（推荐id选择器）
        filter:'bacClassDutyDoctorList_table', ////必填，指定的lay-filter的名字
        //执行渲染table配置
        render:{
            height:'full-135', //table的高度，页面最大高度减去差值
            url: $.config.services.schedule + "/bacClassDuty/listDutyDoctor.do", // ajax的url必须加上$.config.services.schedule方法
            ...
            if(layEvent.indexOf('edit')>-1){ //编辑
                ...
                if(comareDay>dutyDate){
                    ...
                    if(arrs.length>1){
                        ...
                        dutyEdit(data.userId,data.userName,classManageId,dutyDate,"0")
                    }else{
                        dutyEdit(data.userId,data.userName,classManageId,dutyDate,"1")
                    }
                }
            }
        }
...
function getManageList() {
    ...
    _layuiTable({
        ...
        render:{
            height:'full-135', //table的高度，页面最大高度减去差值
            url: $.config.services.schedule + "/bacClassManage/listManageDoctor.do", // ajax的url必须加上getRootPath()方法
            where:param, //接口的参数。如：where: {token: 'sasasas', id: 123}
            page: false, //开启分页
            cols: [[ //表头
                {field: 'className', title: '班种信息',templet: function(d){
...
function classManage(){
    ...
    title="班种管理";
    url=$.config.server+"/backstage/bacClassManageDoctorList";
    ...
    _layerOpen({
        ...
        readonly:true,
            ...
            table.reload('bacClassDutyDoctorList_table'); //重新刷新table
...
function classTemplate() {
    ...
    title="值班模板管理";
    url=$.config.server+"/backstage/bacClassTemplateDoctorList";
 ...
 * @param dutyDate
 */
function dutyEdit(userId,userName,classManageId,dutyDate,editType) {
    var url="";
    var title="";
    title="编辑排班";
    //判断有无编辑权限
    if(!bacClassDutyDoctorList.baseFuncInfo.authorityTag('doctorDutyEdit#edit')){
        return;
    }
    url=$.config.server+"/backstage/doctorDutyEdit?userId="+userId+"&userName="+userName+"&classManageId="+classManageId
    +"&dutyDate="+dutyDate+"&editType="+editType;
    _layerOpen({
        url:url,  //弹框自定义的url，会默认采取type=2
...
function classDuty() {
    ...
    title="考勤统计";
    url=$.config.server+"/backstage/bacClassDutyDoctorReport";
...
function exportExcel(){
    ...
    var name = "医生排班表_" + data.dutyDate_begin + "-" + data.dutyDate_end + ".xlsx";
    _downloadFile({
        url: $.config.services.schedule + "/bacClassDuty/exportDutyDoctor.do",
        data: data,
        fileName: name
    });
}
...
function getSearchParam() {
    var searchParam = layui.form.val("bacClassDutyDoctorList_search");
