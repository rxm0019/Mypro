...
<body ms-controller="bacPatientScheduleList">
<div class="layui-fluid">
    <div class="layui-card" style="margin-bottom: 10px;">
        <!--搜素栏的div-->
        <div class="layui-form layui-card-header layuiadmin-card-header-auto search-form"
             id="bacPatientScheduleList_search" lay-filter="bacPatientScheduleList_search">
        </div>

        <div class="layui-card-body">
            <!--工具栏的按钮的div，注意：需要增加权限控制-->
            <div style="padding: 10px;" id="bacPatientScheduleList_tool">
                <button :visible="@baseFuncInfo.authorityTag('bacPatientScheduleList#add')"
                        class="layui-btn layui-btn-dismain"  onclick="saveOrEdit()">添加</button>
                <button :visible="@baseFuncInfo.authorityTag('bacPatientScheduleList#copy')"
                        class="layui-btn layui-btn-dismain"  onclick="copySchedule()">复制排班</button>
                <button :visible="@baseFuncInfo.authorityTag('bacPatientScheduleList#query')"
                        class="layui-btn layui-btn-dismain"  onclick="querySchedule()">排床查询</button>
                <button :visible="@baseFuncInfo.authorityTag('bacPatientScheduleList#preview')"
                        class="layui-btn layui-btn-dismain"  onclick="monthSchedule()">按月预览</button>
                <button :visible="@baseFuncInfo.authorityTag('bacPatientScheduleList#saveTemplate')"
                        class="layui-btn layui-btn-dismain"  onclick="saveTemplate()">保存模板</button>
                <button :visible="@baseFuncInfo.authorityTag('bacScheduleDetailList#add')"
                        class="layui-btn layui-btn-dismain"  onclick="pushTemplate()">调出模板</button>
                <button :visible="@baseFuncInfo.authorityTag('bacPatientScheduleList#export')"
                        class="layui-btn layui-btn-dissub"  onclick="exportExcel()">导出</button>
            </div>
        </div>
    </div>

    <div class="layui-card" style="padding: 5px 0px 5px 0px;margin-bottom: 10px;">
        <div style="text-align: center" id="weekBtn">
            <button class="layui-btn layui-btn-dismain week-btn"  onclick="lastWeek()">上一周</button>
            ...
            <button class="layui-btn layui-btn-dismain week-btn" onclick="nextWeek()">下一周</button>
        </div>
    </div>

    <div class="layui-card-body calc-week-height">
...
<!--请在下方写此页面业务相关的脚本-->
<script type="text/javascript" src="${ctxsta}/static/js/backstage/bacPatientScheduleList.js?t=${currentTimeMillis}"></script>
</body>
</html>